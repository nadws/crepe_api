...
class dataOrderanController extends Controller
{
    ...
    public function exportOrderan(Request $r)
    {
        $tgl1 = $r->tgl1;
        $tgl2 = $r->tgl2;
        ...
        $tb_order = DB::select("SELECT a.* ,b.nm_menu, c.nm_meja, d.nama AS koki1 , e.nama AS koki2, f.nama AS koki3,
            ...
            LEFT JOIN tb_karyawan AS f ON f.id_karyawan = a.id_koki3
            LEFT JOIN tb_order2 ON a.id_order = tb_order2.id_order1
            WHERE a.tgl between '$tgl1' and '$tgl2' and a.id_lokasi = '$loc'  and a.void = 0 order by a.id_order DESC");
        ...
        $sheet
            ...
            ->setCellValue('E1', 'Time Order')
            ->setCellValue('F1', 'Server')
            ->setCellValue('G1', 'Koki')
            ->setCellValue('H1', 'Waitress')
            ->setCellValue('I1', 'Time Delay')
            ->setCellValue('J1', 'Status');
        
        $i = 1;
        $kolom = 2;
        foreach ($tb_order as $k) {
            $sheet
                ->setCellValue('A'.$kolom,$i++)
                ->setCellValue('B'.$kolom,$k->no_order)
                ->setCellValue('C'.$kolom,$k->nm_meja)
                ->setCellValue('D'.$kolom,$k->nm_menu);
                $waktu1 = new DateTime($k->j_mulai);
            $sheet
                ->setCellValue('E'.$kolom,$waktu1->format('h.i A'))
                ->setCellValue('F'.$kolom,$k->admin)
                ->setCellValue('G'.$kolom,$k->koki1.','.$k->koki2.','.$k->koki3)
                ->setCellValue('H'.$kolom,$k->pengantar)
                ->setCellValue('I'.$kolom,$k->selisih)
                ->setCellValue('J'.$kolom,$k->selesai);
            $kolom++;
        }
        ...
        $batas = count($tb_order) + 1;
        $sheet->getStyle('A1:J'.$batas)->applyFromArray($style);
        
        $lokasi = $loc == 1 ? 'Data Orderan Takemori.xlsx' : 'Data Orderan Soondobu.xlsx';
