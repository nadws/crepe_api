    ...
    public function index(Request $request)
    {
        ...
        if (empty($id_menu)) {
            ...
            $data = [
                ...
                'invoice' => DB::select("SELECT a.*, d.nm_meja, e.bayar
                FROM tb_transaksi AS a
                
                LEFT JOIN (
                    SELECT b.no_order, b.no_order2 FROM  tb_order2 AS b 
                    group by b.no_order2
                ) b ON b.no_order2 = a.no_order
                
                LEFT JOIN (
                SELECT c.no_order, c.id_meja
                    FROM tb_order as c
                    GROUP by c.no_order
                ) c on c.no_order = b.no_order
               
               
                
                LEFT JOIN tb_meja AS d ON d.id_meja = c.id_meja
                LEFT JOIN tb_invoice AS e ON e.no_nota = a.no_order
                WHERE a.tgl_transaksi between '$tgl1' and '$tgl2' and a.id_lokasi = '$loc'
                GROUP BY a.no_order
                order by d.id_meja ASC
                ")
            ];
