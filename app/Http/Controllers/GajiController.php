...
class GajiController extends Controller
{
    ...
    public function gajiExportTemplate(Request $request)
    {
        ...
        $sheet
            ...
            ->setCellValue('H1', 'RP SP')
            ->setCellValue('I1', 'BULANAN')
            ->setCellValue('J1', 'ADD POINT');
        $pos = 2;
        $posisi = Posisi::all();
        foreach ($posisi as $l) {

            $sheet
                ->setCellValue('M' . $pos, $l->id_posisi)
                ->setCellValue('N' . $pos, $l->nm_posisi)
                ->setCellValue('O' . $pos, $l->ket);
            $pos++;
        }
        $kolom = 2;
        $i = 1;
        foreach ($gaji as $k) {
            ...
            $sheet->setCellValue('H' . $kolom, $k->rp_sp);
            $sheet->setCellValue('I' . $kolom, $k->g_bulanan);
            $sheet->setCellValue('J' . $kolom, $k->point);
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Gaji Karyawan TS.xlsx"');
        header('Cache-Control: max-age=0');
    ...
    public function gaji_export_new(Request $r)
    {
        ...
        foreach ($gaji as $l) {
            ...
            $sheet
                ...
                ->setCellValue('O' . $row, number_format($total));
